describe('DoIt design review captures', () => {
	...
	beforeEach(() => {
		...
		cy.intercept('GET', '**/api/calendar/reminders/due*', [])
		cy.intercept('GET', '**/api/calendar/reports/monthly*', { activeEvents: 0, totalEvents: 0, enabledReminders: 0, acknowledgedReminders: 0 })
	...
	it('captures core app surfaces', () => {
		...
		cy.contains('Fregar platos').should('be.visible')
		cy.screenshot('design-before-now-mobile', { capture: 'viewport' })
		cy.contains('button', 'Más').click()
		cy.get('.mobile-dock__more-menu').contains('a', 'Revisión').click()
		cy.get('.app-title-block').contains('Revisión').should('be.visible')
		cy.screenshot('design-review-mobile', { capture: 'viewport' })
		cy.visit('/now')
		...
		cy.screenshot('calendar-create-modal-desktop', { capture: 'viewport' })
		cy.viewport(390, 844)
		cy.get('[role="dialog"]').should('be.visible')
		cy.screenshot('calendar-create-modal-mobile', { capture: 'viewport' })
	})
})
